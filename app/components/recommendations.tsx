  ...
  return (
    <section className="pt-24 md:py-0 md:mb-20 xl:mb-32 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-28">
        ...
        {(recommendationsData as Recommendation[]).map(
              ...
              <div className="bg-stone-100 dark:bg-zinc-800/90 rounded-2xl shadow-2xs dark:shadow-sm border-2 border-stone-200/60 dark:border-zinc-700/50">
                ...
                <div
                  className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 
                          ...
                          z-10"
                >
                  <div className="w-12 h-12 text-purple-900 dark:text-amber-400">
                    {" "}
                    {/* Changed from w-8 h-12 to w-16 h-16 */}
                    {AnimalIcons[recommendation.icon]}
                  </div>
                </div>

                {/* Increase top padding to accommodate larger icon */}
                <div className="pt-16 p-4 md:pt-16 md:p-8">
                  {" "}
                  {/* Changed from pt-10 to pt-16 */}
                  <div className="space-y-4">
                    <div className="text-center">
                      ...
                      {recommendation.email[language].startsWith("mailto:") ? (
                        <a
                          href={recommendation.email[language]}
                          className="text-xs md:text-sm text-purple-600 dark:text-amber-400 mt-0.5
                                  block hover:underline transition-colors break-all md:break-normal"
                        >
                          {recommendation.email[language].replace("mailto:", "")}
                        </a>
                      ) : recommendation.email[language] === "ask for contact details" ? (
                        <Link
                          href="/contact"
                          className="text-xs md:text-sm text-purple-600 dark:text-amber-400 mt-0.5
                        ...
                        <p className="text-xs md:text-sm text-purple-600 dark:text-amber-400 mt-0.5 break-all md:break-normal">
                          {recommendation.email[language]}
                        </p>
                      )}
                    </div>
                    <blockquote
